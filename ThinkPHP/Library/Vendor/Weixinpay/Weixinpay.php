...
class Weixinpay
{
    ...
    private $config = array(
        ...
        'NOTIFY_URL' => 'http://ml.stmingliu.com/index.php/Api/WeixPay/notify/order_number/', // 接收支付状态的连接  改成自己的域名
    );
    ...
    public function getParameters()
    {
        ...
        if (!isset($_GET['code'])) {
            ...
            $data = $Model->query("select price as total_fee from ml_orders where order_id = " . $out_trade_no . " limit 1");
            $total_fee = intval(round(floatval($data[0]['total_fee']) * 100));
            $order = array(
                'body' => '名流商品',// 商品描述（需要根据自己的业务修改）
