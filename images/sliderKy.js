function slideKy(){
 ...
 slideKy.prototype = {
     init:function(opts){
         ...
         this.space = Math.round((this.outerWidth - opt.width) / (opt.disNum - 1)) ;
         this.iSort = [];
         this.tSort = [];
         this.property = [];
         this.timer = null;
         ...
         this.cretaeProperty();
         for(var i=0;i<this.len ;i++){this.iSort.push(this.lis[i]),this.tSort.push(this.text[i])};
         this.iSort.unshift(this.iSort.pop());
         this.tSort.unshift(this.tSort.pop());
         if(opt.auto){
             this.timer = setInterval(function(){_this.doNext();},opt.timer);
         }
         this.itemHandler();
         this.outerHandler();
         this.slider();
         this.prev.onclick = function(){_this.doPrev();}
         this.next.onclick = function(){_this.doNext();}
     },
     outerHandler:function(){
         ...
         this.outer.onmouseout = function(){
             if(opt.auto){
                 _this.timer = setInterval(function(){_this.doNext();},opt.timer);
             }
         }
     },
     ...
     css:function(el,key,value){
         if(arguments.length === 3){
             ...
             if(typeof key == 'string'){
                 key = key.replace(/-\w/g,function(a){return a.charAt(1).toUpperCase()});
     ...
     animate:function(el,opts,callback,aniFn){
         ...
         opts = opts || {};
         var _this = this;
         var deStyle = {},D = Date,d = new D,e,T=500,o, F = function(x){ return (aniFn || _easeInOutCubic)(x)};
         for(var i in opts){
